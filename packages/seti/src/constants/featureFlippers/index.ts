...
function getFeatureFlippersByEnv(env: Env) {
  return Object.entries(featureFlippersObject).reduce<FlipperMap>(
    (featureFlippers, [key, value]) => {
      featureFlippers[key as FeatureFlippersKeys] =
        env === "test" ? value.dev : value[env];
...
export default getFeatureFlippersByEnv(
  process.env.NEXT_PUBLIC_RUNTIME_ENV as Env
);
