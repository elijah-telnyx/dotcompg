...
export interface MetaTagsProps {
  ...
  locales?: { items: { name?: string; value: string; href: string; metatagAlpha2?: string } }[];
  page?: string;
}

const addCountryPrefixFromAlpha2 = (alpha2: string) => (copy: string) => {
  if (!alpha2) return copy;

  const countryData = getCountryByAlpha2(alpha2);
  if (!countryData) {
    return copy;
  }
  return `${countryData?.name} ${copy}`.trimStart();
};
...
const MetaTags = ({
  ...
  const addCountryPrefix = addCountryPrefixFromAlpha2(currentLocale);
  const addPage = addPageToCopy(page);

  const extendCopy = (copy: string, { pageSuffix }: { pageSuffix?: boolean } = {}) => {
    return addPage(addCountryPrefix(copy), pageSuffix);
  };
  ...
  const generateHreflangTags = () =>
    hreflangTags.map((i, k) => <link key={k} rel='alternate' hrefLang={i.language} href={i.url} />);
  ...
  return (
    <Head>
      ...
      <meta key='meta-og:site_name' property='og:site_name' content='Telnyx' />

      <meta key='meta-og:title' property='og:title' content={og.title} />
      <meta key='meta-og:description' property='og:description' content={og.description} />
      {og.type && <meta key='meta-og:type' property='og:type' content='article' />}
      {og.image && (
        <>
          <meta key='meta-og:image' property='og:image' content={og.image.src} />
          <meta key='meta-og:image-alt' property='og:image:alt' content={og.image.alt} />
          <meta key='meta-og:image-width' property='og:image:width' content={String(og.image.width)} />
          <meta key='meta-og:image-height' property='og:image:height' content={String(og.image.height)} />
        </>
      )}
      ...
      <meta key='meta-twitter:site' name='twitter:site' content='@telnyx' />
      <meta key='meta-twitter:title' name='twitter:title' content={twitter.title} />
      <meta key='meta-twitter:description' name='twitter:description' content={twitter.description} />
      {twitter.image && (
        <>
          <meta key='meta-twitter:image' name='twitter:image' content={twitter.image.src} />
          <meta key='meta-twitter:image-alt' name='twitter:image:alt' content={twitter.image.alt} />
        </>
      )}

      {schema && (
        <script id='page-schema' type='application/ld+json' dangerouslySetInnerHTML={{ __html: sanitize(schema) }} />
      )}
    </Head>
  );
};
