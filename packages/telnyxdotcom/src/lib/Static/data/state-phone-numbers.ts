import stateNumberData, { type state, BLOGARTICLECONSTANTS } from './stateNumberData';
import { testimonials } from './countryNumberData';
...
const hero = async ({ name, demonym }: state) => {
  const state_code = stateNumberData.find((state) => state.name === name)?.alpha2;
  const media = await getPhoneNumberHeroMedia({ state_code, name });
...
const sections = ({ name, areaCodes, demonym, topCities, available, metro }: state, stateNumberCount: string) => {
  const faqAreaCodeCopy = () => {
    const genCopy = (codes: string) => `${name} has area codes for ${codes}.`;
    switch (areaCodes.length) {
      case 1:
        return genCopy(areaCodes[0]);
      case 2:
        return genCopy(`${areaCodes[0]} and ${areaCodes[1]}`);
      default:
        return genCopy(`${areaCodes.slice(0, -1).join(', ')} and ${areaCodes.slice(-1)}`);
    }
  };
  ...
  if (+stateNumberCount) {
    cardItems.push({
      ...
      highlightTitle: FormattedNumberCopy(stateNumberCount),
  ...
  if (+available) {
    cardItems.push({
      ...
      highlightTitle: FormattedNumberCopy(available),
  ...
  return {
    links: {
      title: 'Buy State Numbers',
      directoryLinksTabs: directoryLinks(stateNumberData, name),
      currentDirectory: getFirstCharacter(name),
    } as LinksDirectorySectionProps,
    metro: {
      heading: `${name} Availability`,
      copy: `${name}'s country code is +1. Local numbers are available in multiple area codes from Telnyx.`,
      tagline: `AVAILABILITY`,
      metro,
    } as AvailabilitySectionProps | undefined,
    logos: {
      title: 'State Numbers - Logos',
    ...
    whyTelnyx: {
      title: 'State Numbers - Why Telnyx',
      tagline: 'WHY TELNYX',
    ...
    howItWorks: {
      title: 'State Numbers - How it works',
      tagline: `Purchase ${name} numbers`,
      headingElement: 'h2',
      items: [
        {
          title: 'State Numbers - How it works 1',
        ...
        {
          title: 'State Numbers - How it works 2',
        ...
        {
          title: 'State Numbers - How it works 3',
        ...
        {
          title: 'State Numbers - How it works 4',
    ...
    faqs: {
      ...
      title: 'State Numbers - FAQ',
      tagline: 'FAQ',
      questions: [
        {
          id: '2MBuKJytElk123b7H39cC9',
          title: 'State Numbers - FAQ 1',
        ...
        },
        {
          ...
          title: 'State Numbers - FAQ 2',
          question: `How do I get a ${name} virtual number?`,
          answer: `It’s so easy. Sign up for the Mission Control Portal with Telnyx, use our portal to find the perfect number, connect your ${name} number for use, and start dialing from recognizable local numbers in ${topCities[0]}, ${topCities[1]} and ${topCities[2]}.`,
        },
        {
          id: '2tt5QLmKFGhjdaab7DDY5e',
          title: 'State Numbers - FAQ 3',
          question: `What are ${name}’s local area code(s)?`,
          answer: faqAreaCodeCopy(),
        },
      ],
      backgroundColor: 'cream',
      spacingTop: 'contrasting',
    ...
    airTableContent: {
      localEconomy: null as { heading: string; copy: string } | null,
      localRegulations: null as { heading: string; copy: string } | null,
    },
    testimonials: {
...
const pageData = async (slug: string, stateNumberCount: string) => {
  const state = stateNumberData.find((i: state) => i.slug === slug);
  return state
    ? {
        state,
        seo: seo(state),
        hero: await hero(state),
        sections: sections(state, stateNumberCount),
