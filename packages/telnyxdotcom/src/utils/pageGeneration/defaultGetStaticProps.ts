...
export interface Params {
  page: string;
  getData: (context: GetStaticPropsContext<ContextProps>) => Promise<unknown> | unknown;
  scripts?: {
    pipedata?: boolean;
  };
}

export const defaultGetStaticProps = <PageEntryProps extends { [key: string]: any }>({
  page,
  getData,
  scripts = {},
}: Params) => {
  const nextJSGetStaticProps = async (context: GetStaticPropsContext<PageEntryProps, ContextProps>) => {
    if (!getData)
      throw new Error(
    ...
    try {
      const pageEntry = (await getData({ ...context, preview })) as PageEntryProps;
