...
const pageName = 'global-edge-router';
const getData = fetchPricingPages[pageName];

type PricingFaxApiProps = Awaited<ReturnType<typeof getData>> & {
  preview: boolean;
};
...
const PricingSlugPage = ({ hero, sections, tablesData, preview }: PricingFaxApiProps) => {
  return (
    <>
        ...
        if (sectionType === 'sectionPricingTable') {
          return (
            <ErrorBoundary key={section?.id} preview={preview}>
              <TablesSection {...section} data={tablesData} />
            </ErrorBoundary>
          );
        }
...
export const getStaticProps = defaultGetStaticProps<PricingFaxApiProps>({
